...
export default function WhatsApp() {
  ...
  const updateConfigMutation = useUpdateWhatsAppConfig({
    mutation: { onSuccess: () => toast({ title: "Config saved" }) }
  });
  ...
  return (
    <PageTransition className="h-[calc(100vh-6rem)] flex flex-col space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl">WhatsApp Integration</h1>
          <p className="text-muted-foreground mt-1">Communicate with drivers and clients.</p>
        </div>
      ...
      {activeTab === "chat" ? (
        <Card className="flex-1 flex overflow-hidden">
          ...
          <div className="w-80 border-r border-white/10 bg-white/5 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {contacts.length === 0 && <div className="p-4 text-sm text-muted-foreground text-center">No contacts yet</div>}
            </div>
          </div>
            ...
            {selectedContact ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col-reverse">
                  {/* flex-col-reverse assumes API returns newest first, if oldest first, remove reverse and handle scrolling */}
                  <div className="flex flex-col space-y-4">
                    {messages.slice().reverse().map(msg => {
                      const isOutbound = msg.direction === 'outbound';
                      return (
                        <div key={msg.id} className={`flex max-w-[70%] ${isOutbound ? 'self-end' : 'self-start'}`}>
        ...
        <Card className="max-w-2xl mx-auto p-8 w-full">
          <div className="flex items-center gap-3 mb-8 pb-4 border-b border-white/10">
            ...
            <h2 className="text-xl font-display">Meta API Configuration</h2>
          </div>
          <form onSubmit={handleSubmit((d) => updateConfigMutation.mutate({ data: d as any }))} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">Use this exact string when setting up the webhook in Meta Developer portal.</p>
            </div>
